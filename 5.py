...
class App:
    ...
    def analysis(self, flag):
        ...
        if not flag:
            ...
            while video_has_more and self.status_download:
                ...
                # req['aweme_list']中返回的是一个列表，其中元素是json数组，需要再次对这个数组进行解析，提取到正确的video_url
    ...
    def download_video(self, video, num=0):
        try:
            ...
            vid = video['video']['play_addr']['uri']  # video link
            caption = re.sub('[ \\/:*?"<>|\n\t]', '', caption)
            ...
            if caption:
                download_url = f'https://aweme.snssdk.com/aweme/v1/play/?video_id={vid}&ratio=1080p'
                video_data = self._requests('get', download_url, decode_level=3).content
                self.save_video(os.path.normpath(filepath),
                                f'{num:0>3d}_' + caption + '_' + vid + '.mp4', video_data,
                                download_url)
